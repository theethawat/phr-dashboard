...
class Spo2Stat extends Component<any, any> {
    ...
    componentDidMount() {
        firestore.collection("spo2").get().then(snapshot => {
            if (snapshot.empty) {
            ...
            snapshot.forEach(doc => {
                ...
                if (data.pulseOximeter < 93)
                    danger++
                else if (data.pulseOximeter >= 93 && data.pulseOximeter <= 96)
                    risk++
                else if (data.pulseOximeter > 96)
                    safe++
    ...
    render() {
        ...
        return (<div className="card">
            <div className="card-content">
                <h4 className="subtitle is-5">สถิติระดับออกซิเจนในเลือด</h4>
