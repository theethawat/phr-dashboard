    ...
    firestore
      ...
      .then((snapshot) => {
        ...
        snapshot.forEach((doc) => {
          ...
          if (tempGlucose.value * 100000 <= 100) safeCollect++
          if (
            tempGlucose.value * 100000 > 100 &&
            tempGlucose.value * 100000 <= 125
          )
            intermediateCollect++
          if (tempGlucose.value * 100000 > 125) dangerCollect++
          this.setState({
      ...
      <div className="card">
        ...
        <div className="card-content">
          ...
          <progress
            ...
            value={this.state.safeZone as number}
            max={stateGlucose.length}
          />

          <label className="label">
            เกณฑ์เสี่ยงเบาหวาน {this.state.intermediateZone} รายการ ({" "}
            {this.percentCalculate(
              this.state.intermediateZone as number,
              stateGlucose.length
            ).toFixed(2)}{" "}
            % ){" "}
          </label>
          ...
          <progress
            ...
            value={this.state.intermediateZone as number}
            max={stateGlucose.length}
          />

          <label className="label">
            เกณฑ์เป็นเบาหวาน {this.state.dangerZone} รายการ ({" "}
            {this.percentCalculate(
              this.state.dangerZone as number,
              stateGlucose.length
            ).toFixed(2)}{" "}
            % ){" "}
          </label>
          ...
          <progress
            ...
            value={this.state.dangerZone as number}
            max={stateGlucose.length}
          />
        </div>
      </div>
