...
class PressureStat extends Component<any, any>{
    ...
    componentDidMount() {
        ...
        firestore.collection("blood_pressure").get().then(snapshot => {
            if (snapshot.empty) {
    ...
    render() {
        ...
        return (<div className="card">
            <div className="card-content">
                <h4 className="subtitle is-5">สถิติความดันโลหิต</h4>
                ...
                <label className="label">
                    ...
                    ({this.percentCalculate(pressureData.systolic.safe, amount).toFixed(2)} % )
                </label>

                <progress className="progress is-success" value={pressureData.systolic.safe} max={amount} />

                <label className="label">
                    เกณฑ์เสี่ยง {pressureData.systolic.risk}  รายการ
                    ({this.percentCalculate(pressureData.systolic.risk, amount).toFixed(2)} % ) </label>

                <progress className="progress is-warning" value={pressureData.systolic.risk} max={amount} />

                <label className="label">
                    เกณฑ์อันตราย {pressureData.systolic.danger}รายการ
                    ({this.percentCalculate(pressureData.systolic.danger, amount).toFixed(2)} % ) </label>

                <progress className="progress is-danger" value={pressureData.systolic.danger} max={amount} />
                ...
                <label className="label">
                    ...
                    ({this.percentCalculate(pressureData.diastolic.safe, amount).toFixed(2)} % )
                  </label>

                <progress className="progress is-success" value={pressureData.diastolic.safe} max={amount} />

                <label className="label">
                    เกณฑ์เสี่ยง {pressureData.diastolic.risk} รายการ
                    ({this.percentCalculate(pressureData.diastolic.risk, amount).toFixed(2)} % )
                </label>

                <progress className="progress is-warning" value={pressureData.diastolic.risk} max={amount} />

                <label className="label">
                    เกณฑ์อันตราย {pressureData.diastolic.danger} รายการ
                    ({this.percentCalculate(pressureData.diastolic.danger, amount).toFixed(2)} % )
                </label>

                <progress className="progress is-danger" value={pressureData.diastolic.danger} max={amount} />
